...
class SwitchbotAdvertising {
  ...
  parse(peripheral, onlog) {
    ...
    if (model === "H") {
      ...
      // Note: Unofficial impelment
      sd = this._parseServiceDataForWoIOSensorTH(buf, manufacturerData, onlog);// WoIOSensorTH
    } else {
  ...
  // Note: Unofficial impelment
  _parseServiceDataForWoIOSensorTH(buf, manData, onlog) {
      if (buf.length !== 3) {
        if (onlog && typeof onlog === "function") {
        onlog(
          `[_parseServiceDataForWoIOSensorTH] Buffer length ${buf.length} !== 3!`
        );
      }
      return null;
    }
    if (manData.length !== 14) {
      if (onlog && typeof onlog === "function") {
        onlog(
          `[_parseServiceDataForWoIOSensorTH] Data length ${manData.length} !== 14!`
        );
      }
      return null;
    }
    let byte10 = manData.readUInt8(10);
    let byte11 = manData.readUInt8(11);
    let byte12 = manData.readUInt8(12);

    let temp_sign = byte11 & 0b10000000 ? 1 : -1;
    let temp_c = temp_sign * ((byte11 & 0b01111111) + (byte10 & 0b00001111) / 10);
    let temp_f = (temp_c * 9 / 5) + 32;
    temp_f = Math.round(temp_f * 10) / 10;
    ...
    let data = {
      ...
      fahrenheit: byte12 & 0b10000000 ? true : false,
      humidity: byte12 & 0b01111111,
      //battery: byte2 & 0b01111111,
      battery: buf[2]  & 0b01111111, // TODO: I don't know if it's right
